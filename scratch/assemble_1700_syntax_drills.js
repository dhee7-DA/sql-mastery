// Assembly script for Section 0: The SQL Syntax Gym (1,700 Micro-Drills)
// Combines Topics 1 through 17 into visualizer/syntax_gym_data.js
...
const t17 = JSON.parse(fs.readFileSync('scratch/syntax_drills_t17.json', 'utf8'));

console.log(`Loaded drills: T1=${t1.length}, T2=${t2.length}, T3=${t3.length}, T4=${t4.length}, T5=${t5.length}, T6=${t6.length}, T7=${t7.length}, T8=${t8.length}, T9=${t9.length}, T10=${t10.length}, T11=${t11.length}, T12=${t12.length}, T13=${t13.length}, T14=${t14.length}, T15=${t15.length}, T16=${t16.length}, T17=${t17.length}`);

const all1700 = [...t1, ...t2, ...t3, ...t4, ...t5, ...t6, ...t7, ...t8, ...t9, ...t10, ...t11, ...t12, ...t13, ...t14, ...t15, ...t16, ...t17];

// Verify drill numbering is contiguous 1..1700
all1700.forEach((d, idx) => {
  d.drillNumber = idx + 1;
});

const fileContent = `// =============================================================================
// SECTION 0: THE SQL SYNTAX GYM (1,700 Progressive In-Depth Micro-Drills)
// Topics 1–17:
...
//  17. DDL & DML Schema Modifications (#1601–#1700)
// =============================================================================

const SYNTAX_GYM_DRILLS = ${JSON.stringify(all1700, null, 2)};
...
console.log(`Successfully assembled visualizer/syntax_gym_data.js with ${all1700.length} drills!`);
