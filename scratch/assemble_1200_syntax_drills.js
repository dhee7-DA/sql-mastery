// Assembly script for Section 0: The SQL Syntax Gym (1,200 Micro-Drills)
// Combines Topics 1 through 12 into visualizer/syntax_gym_data.js
...
console.log(`Loaded drills: T1=${t1.length}, T2=${t2.length}, T3=${t3.length}, T4=${t4.length}, T5=${t5.length}, T6=${t6.length}, T7=${t7.length}, T8=${t8.length}, T9=${t9.length}, T10=${t10.length}, T11=${t11.length}, T12=${t12.length}`);

const all1200 = [...t1, ...t2, ...t3, ...t4, ...t5, ...t6, ...t7, ...t8, ...t9, ...t10, ...t11, ...t12];

// Verify drill numbering is contiguous 1..1200
all1200.forEach((d, idx) => {
  d.drillNumber = idx + 1;
});
...
const SYNTAX_GYM_DRILLS = ${JSON.stringify(all1200, null, 2)};
...
console.log(`Successfully assembled visualizer/syntax_gym_data.js with ${all1200.length} drills!`);
